 ...
 */
      ...
      ignore Array.map<GT.Neuron, Text>(
        ...
        func(thisItem) : Text {
          ...
          h.put(
            ...
            #Number(
              Nat64.toNat(
                switch (Option.get<GT.DissolveState>(thisItem.dissolve_state, #DissolveDelaySeconds(0))) {
  ...
  */
  ...
  public shared ({ caller }) func icrc1_transfer(args : ICRC1.TransferArgs) : async ICRC1.TransferResult {
    ...
    switch (result) {
      case (#Ok(val)) {
        if (args.to.subaccount == null) {
          //only null subaccounts can be members of the dao
  ...
  /**
    ...
    */
  public shared ({ caller }) func update_token(
    request : {
  ...
  public shared ({ caller }) func force_refresh_balance(request : [Principal]) : async Result.Result<Bool, Text> {
    ...
    label process for (thisItem in request.vals()) {
      let encodedAccount = ICRC1Account.encode({
        owner = thisItem;
        subaccount = null;
      });
