...
shared ({ caller = creator }) actor class AxonService() = this {
  ...
  let #v2_1_1(#data(state_current)) = migration_state else Debug.trap("Migration state is not up to date");
  ...
  */
  private func _mint(caller : Principal, axonId : Nat, p : Principal, a : Nat) : async* CurrentTypes.Result<CurrentTypes.AxonCommandExecution> {
    ...
    let command : CurrentTypes.AxonCommandRequest = #Mint({
      amount = a;
      recipient = ?p;
    });
 ...
 */
  private func _burn(caller : Principal, axonId : Nat, p : Principal, a : Nat) : async* CurrentTypes.Result<CurrentTypes.AxonCommandExecution> {
    ...
    let command : CurrentTypes.AxonCommandRequest = #Burn({
      amount = a;
      owner = p;
    });
    var maybeNewAxon : ?CurrentTypes.AxonFull = null;
 ...
 */ public query func topAxons() : async [CurrentTypes.AxonPublic] {
    let filtered = Array.mapFilter<CurrentTypes.AxonFull, CurrentTypes.AxonPublic>(
      SB.toArray(state_current.axons),
      func(axon) {
    ...
    Array.sort<CurrentTypes.AxonPublic>(
      filtered,
      func(a, b) {
 ...
 */
  public query ({ caller }) func ledger(id : Nat) : async [CurrentTypes.LedgerEntry] {
    let { ledger } = SB.get(state_current.axons, id);
    // sort descending
    Array.sort<CurrentTypes.LedgerEntry>(
      Iter.toArray(Map.entries<Principal, Nat>(ledger)),
      func(a, b) {
 ...
 */
  public query ({ caller }) func myAxons() : async [CurrentTypes.AxonPublic] {
    Array.mapFilter<CurrentTypes.AxonFull, CurrentTypes.AxonPublic>(
      SB.toArray<CurrentTypes.AxonFull>(state_current.axons),
      func(axon) {
        switch (Map.get(axon.ledger, phash, caller)) {
          case (?balance) {
            if (balance > 0) { ?getAxonPublic(axon) } else { null };
          };
          case _ { null };
        };
      },
  ...
  public query ({ caller }) func getProposalById(axonId : Nat, proposalId : Nat) : async CurrentTypes.Result<CurrentTypes.AxonProposalPublic> {
    ...
    switch (Array.find<CurrentTypes.AxonProposal>(SB.toArray(activeProposals), func(p) { p.id == proposalId })) {
      case (?found) {
        #ok({
          found with
          ballots = Iter.toArray(Iter.map<CurrentTypes.Ballot, CurrentTypes.BallotPublic>(Map.vals<Principal, CurrentTypes.Ballot>(found.ballots), func(x) { { votingPower = x.votingPower; vote = x.vote; principal = x.principal; voted_by = x.voted_by } }));
      ...
      case _ {
        ...
        switch (item) {
          ...
          case (?item) #ok {
            item with
            ballots = Iter.toArray(Iter.map<CurrentTypes.Ballot, CurrentTypes.BallotPublic>(Map.vals<Principal, CurrentTypes.Ballot>(item.ballots), func(x) { { votingPower = x.votingPower; vote = x.vote; principal = x.principal; voted_by = x.voted_by } }));
 ...
 */
  public query ({ caller }) func getActiveProposals(id : Nat) : async CurrentTypes.ProposalResult {
    ...
    #ok(
      Array.map<CurrentTypes.AxonProposal, CurrentTypes.AxonProposalPublic>(
        SB.toArray<CurrentTypes.AxonProposal>(activeProposals),
        func(p) : CurrentTypes.AxonProposalPublic {
          {
            p with
            ballots = Iter.toArray(Iter.map<CurrentTypes.Ballot, CurrentTypes.BallotPublic>(Map.vals<Principal, CurrentTypes.Ballot>(p.ballots), func(x) { { votingPower = x.votingPower; vote = x.vote; principal = x.principal; voted_by = x.voted_by } }));
 ...
 */
  public query ({ caller }) func getAllProposals(id : Nat, before : ?Nat) : async CurrentTypes.ProposalResult {
    ...
    #ok(
      Array.tabulate<CurrentTypes.AxonProposalPublic>(
        Nat.min(100, size),
        func(i) {
          ...
          {
            items with
            ballots = Iter.toArray(Iter.map<CurrentTypes.Ballot, CurrentTypes.BallotPublic>(Map.vals<Principal, CurrentTypes.Ballot>(items.ballots), func(x) { { votingPower = x.votingPower; vote = x.vote; principal = x.principal; voted_by = x.voted_by } }));
 ...
 */
  public query ({ caller }) func getMotionProposals(id : Nat) : async CurrentTypes.ProposalResult {
    ...
    let filtered = Array.mapFilter<CurrentTypes.AxonProposal, CurrentTypes.AxonProposalPublic>(
      SB.toArray(allProposals),
      func(p) {
        switch (p.proposal) {
 ...
 */
  public shared ({ caller }) func propose(request : CurrentTypes.NewProposal) : async CurrentTypes.Result<CurrentTypes.AxonProposalPublic> {
    ...
    let ballots = Map.fromIter<Principal, CurrentTypes.Ballot>(
      Iter.map<CurrentTypes.LedgerEntry, (Principal, CurrentTypes.Ballot)>(
        eligibleVoters.vals(),
        func((p : Principal, n : Nat)) {
          (
            p,
            {
              var voted_by = null;
              ...
              var vote = if (p == caller or Set.has<Principal>(delegateSet, phash, p)) {
                ?(#Yes);
              } else { null };
            },
          );
    ...
    let timeStart = clamp(
      ...
      now,
      now + secsToNanos(MAXIMUM_FUTURE_START),
    );

    // Create the proposal, count ballots, then execute if conditions are met
    let newProposal : CurrentTypes.AxonProposal = A._applyExecutingStatusConditionally(A._applyNewStatus({ id = axon.lastProposalId; timeStart = timeStart; timeEnd = timeStart + secsToNanos(clamp(Option.get(request.durationSeconds, DEFAULT_DURATION_SEC), MINIMUM_DURATION_SEC, MAXIMUM_DURATION_SEC));

    ballots = ballots;

    totalVotes = A._countVotes(ballots); creator = caller; proposal = request.proposal; status = SB.fromArray<CurrentTypes.Status>([#Created(now)]); policy = axon.policy }), request.execute == ?true and timeStart <= now);
    ...
    #ok({
      newProposal with
      ballots = Iter.toArray(Iter.map<CurrentTypes.Ballot, CurrentTypes.BallotPublic>(Map.vals<Principal, CurrentTypes.Ballot>(newProposal.ballots), func(x) { { votingPower = x.votingPower; vote = x.vote; principal = x.principal; voted_by = x.voted_by } }));
  ...
  public shared ({ caller }) func vote(request : CurrentTypes.VoteRequest) : async CurrentTypes.Result<()> {
    ...
    label search for (p in SB.vals(axon.activeProposals)) {
      ...
      */
      ...
      Map.forEach<Principal, CurrentTypes.Ballot>(
        p.ballots,
        func(principal : Principal, b : CurrentTypes.Ballot) {
      ...
      proposal := ?A._applyExecutingStatusConditionally(A._applyNewStatus({ p with
      totalVotes = A._countVotes(p.ballots) }), true);
    ...
    if (Result.isOk(result)) {
      ...
      let active_proposals = switch (A.currentStatus(updatedProposal.status)) {
        ...
        case (#Rejected(_)) {
          ...
          SB.fromArray<CurrentTypes.AxonProposal>(
            Array.filter<CurrentTypes.AxonProposal>(
              SB.toArray(axon.activeProposals),
              func(p) {
 ...
 */
  public shared ({ caller }) func cancel(axonId : Nat, proposalId : Nat) : async CurrentTypes.Result<CurrentTypes.AxonProposalPublic> {
    ...
    #ok({
      proposal with
      ballots = Iter.toArray(Iter.map<CurrentTypes.Ballot, CurrentTypes.BallotPublic>(Map.vals<Principal, CurrentTypes.Ballot>(proposal.ballots), func(x) { { votingPower = x.votingPower; vote = x.vote; principal = x.principal; voted_by = x.voted_by } }));
  ...
  */
  public shared ({ caller }) func execute(axonId : Nat, proposalId : Nat) : async CurrentTypes.Result<CurrentTypes.AxonProposalPublic> {
    ...
    let activeProposals = Array.map<CurrentTypes.AxonProposal, CurrentTypes.AxonProposal>(
      SB.toArray(axon.activeProposals),
      func(p) {
        if (p.id != proposalId) {
          return p;
  ...
  public shared ({ caller }) func sync(id : Nat) : async CurrentTypes.NeuronsResult {
    ...
    SB.put(
      ...
      id,
      {
        ...
        var supply = axon.supply;
        neurons = ?neurons;
        totalStake = Array.foldLeft<GT.Neuron, Nat>(
  ...
  */
  public shared ({ caller }) func cleanup(axonId : Nat) : async CurrentTypes.Result<()> {
    ...
    let updatedActiveProposals = Array.map<CurrentTypes.AxonProposal, CurrentTypes.AxonProposal>(
      SB.toArray(axon.activeProposals),
      func(proposal) {
    ...
    let filteredActiveProposals = Array.filter<CurrentTypes.AxonProposal>(
      updatedActiveProposals,
      func(proposal) {
  ...
  func _doExecute(axonId : Nat, proposal : CurrentTypes.AxonProposal) : async CurrentTypes.AxonProposalPublic {
    ...
    SB.put(
      ...
      axon.id,
      {
        axon with
        activeProposals = SB.fromArray<CurrentTypes.AxonProposal>(
          Array.map<CurrentTypes.AxonProposal, CurrentTypes.AxonProposal>(
            SB.toArray(axon.activeProposals),
            func(p) {
              if (p.id == proposal.id) { startedProposal } else p;
            },
          )
        );
        var supply = axon.supply;
    ...
    switch (startedProposal.proposal) {
      case (#NeuronCommand((command, _))) {
        ...
        for (id in specifiedNeuronIds.vals()) {
          ...
          try {
            let response = await axon.proxy.manage_neuron({
              id = ?{ id = id };
...

    {
      ...
      ballots = Iter.toArray(Iter.map<CurrentTypes.Ballot, CurrentTypes.BallotPublic>(Map.vals<Principal, CurrentTypes.Ballot>(startedProposal.ballots), func(x) { { votingPower = x.votingPower; vote = x.vote; principal = x.principal; voted_by = x.voted_by } }));
  ...
  func _applyAxonCommand(axon : CurrentTypes.AxonFull, request : CurrentTypes.AxonCommandRequest) : async* CurrentTypes.Result<(Nat, CurrentTypes.AxonCommandExecution)> {
    switch (request) {
      ...
      case (#AddMembers(principals)) {
        switch (axon.policy.proposers) {
          case (#Closed(current)) {
            let diff = Array.filter<Principal>(
              principals,
              func(p) {
          ...
          axon.id,
          {
            ...
            policy = {
              ...
              acceptanceThreshold = switch (fresh_axon.policy.acceptanceThreshold) {
                case (#Absolute(n)) {
                  #Absolute(A.scaleByFraction(n, to, from));
                };
                case (p) { p };
              };
            };
      ...
      case (#Mint({ amount; recipient })) {
        ...
        */
        ...
        switch (await proxy.mint({ to = { owner = Option.get(recipient, Principal.fromActor(proxy)); subaccount = null }; amount = amount; memo = null; created_at_time = null })) {
          case (#Err(err)) {
      ...
      case (#Burn({ amount; owner })) {
        ...
        switch (await proxy.burn({ from = { owner = owner; subaccount = null }; amount = amount; memo = null; created_at_time = null })) {
          case (#Err(err)) {
            Debug.print(debug_show (err));
            #err(#Error({ error_message = debug_show (err); error_type = #canister_error }));
          };
      ...
      case (#BurnAll({ owner })) {
        ...
        switch (await proxy.burn({ from = { owner = owner; subaccount = null }; amount = current_balance; memo = null; created_at_time = null })) {
          case (#Err(err)) {
            Debug.print(debug_show (err));
            #err(#Error({ error_message = debug_show (err); error_type = #canister_error }));
          };
        ...
        for (thisItem in items.vals()) {
          switch (thisItem) {
            case (#Mint(val)) {
              ...
              requestBuffer.add(#Mint({ to = { owner = Option.get(val.owner, Principal.fromActor(proxy)); subaccount = null }; amount = val.amount; memo = null; created_at_time = null }));
            };
            case (#Burn(val)) {
              ...
              if (current_balance > 0) {
                finalSupplyChange -= switch (val.amount) {
                  case (?val) { val };
                  case (null) {
                    switch (Map.get<Principal, Nat>(axon.ledger, phash, val.owner)) {
                      case (null) current_balance;
                      case (?val) val;
                    };
                  };
                };
                requestBuffer.add(#Burn({ from = { owner = val.owner; subaccount = null }; amount = val.amount; memo = null; created_at_time = null }));
              };
            };
            case (#Balance(val)) {
              let current_balance = Option.get(Map.get(axon.ledger, phash, val.owner), 0);
              if (current_balance != val.amount) {
                finalSupplyChange -= if (current_balance > val.amount) {
                  requestBuffer.add(#Balance({ owner = { owner = val.owner; subaccount = null }; amount = val.amount; memo = null; created_at_time = null }));
      ...
      case (#Transfer({ amount; recipient })) {
        ...
        switch (await proxy.transfer({ from_subaccount = null; to = { owner = recipient; subaccount = null }; amount = amount; memo = null; created_at_time = null })) {
          case (#Err(err)) {
            Debug.print(debug_show (err));
            #err(#Error({ error_message = debug_show (err); error_type = #canister_error }));
          };
          case (#Ok(val)) {
            ...
            #ok(axon.id, #Transfer({ receiver = recipient; amount = amount; senderBalanceAfter = senderBalance - amount }));
          };
        };
      };
    };
  ...
  */
  func _tryGetProposal(id : Nat64, tries : Nat) : async (CurrentTypes.Result<?GT.ProposalInfo>, Nat) {
    try {
      ...
      if (tries >= 10) { (#err(#ProposalNotFound), tries) } else {
