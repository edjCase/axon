...
module {
  ...
  public func upgrade(prev_migration_state : MigrationTypes.State, args : MigrationTypes.Args) : MigrationTypes.State {
    ...
    for (thisItem in args.init_axons.vals()) {
      let aLedger = Map.new<Principal, Nat>(Principal.hash, Principal.equal, func() : Principal = Principal.fromText("2vxsx-fae"));
      ...
      for (thisProposal in thisItem.activeProposals.vals()) {
        SB_lib.add<v2_0_1_types.AxonProposal>(
          activeProposals,
          {
            thisProposal with
      ...
      SB_lib.add<v2_0_1_types.AxonFull>(
        axons,
        {
    ...
    return #v2_0_1(#data({ var axons = axons; var admins = switch (args.init_admins) { case (null) { SB_lib.init<Principal>() }; case (?val) SB_lib.fromArray<Principal>(val.admins) }; var creator = args.creator }));
  };
